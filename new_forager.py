...
def generate_yaml(
    ...
    global ideal_symbols
    ideal_symbols = sorted(ideal_longs)
...
async def optimize(symbols):
    today = datetime.now().date()
    weeks_before = today- timedelta(weeks=4)
    formated_date = weeks_before.strftime('%Y-%m-%d')
    lista_symboli = []
    for sym in symbols:
...
async def dump_yaml(cc, config):
    ...
    global ideal_symbols
    await optimize(ideal_symbols)
    print("koniec optymalizowania")
    lista_symboli = []
    for sym in ideal_symbols:
        lista_symboli.append(sym)
    lista_symboli_str = ','.join(lista_symboli)
    subprocess.run(["rm", "-rf", "configs/live/multisymbol/"])
    subprocess.run(["mkdir", "configs/live/multisymbol/"])
